...
func TestCaptcha(t *testing.T) {
	...
	_, code, err := New(options...).drawStaticImage()
	assert.Error(t, err)
	assert.Len(t, code, 0)

	// error case
	// the expected length of customCode is 4
	_, code, err = New().CustomCode("coo").drawStaticImage()
	assert.Error(t, err)
	assert.Len(t, code, 0)
	...
	_, code, err = New().drawStaticImage()
	assert.Nil(t, err)
	assert.Len(t, code, 4)
	...
	_, code, err = c.CustomCode("cool").drawStaticImage()
	assert.Nil(t, err)
	assert.Equal(t, code, "COOL")

	_, code, err = c.drawStaticImage()
	assert.Nil(t, err)
	assert.Len(t, code, 4)
}
